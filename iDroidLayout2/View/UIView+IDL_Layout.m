...
static char firstLayoutRequestedKey;
...
- (void)setupFromAttributes:(NSDictionary *)attrs {
    ...
    if (clickable != nil) {
        self.userInteractionEnabled = BOOLFromString(clickable);
    }
...
- (BOOL)firstLayoutRequested {
    NSNumber *value = objc_getAssociatedObject(self, &firstLayoutRequestedKey);
    return [value boolValue];
}

- (void)setFirstLayoutRequested:(BOOL)isRequested {
    objc_setAssociatedObject(self,
                             &firstLayoutRequestedKey,
                             @(isRequested),
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)layoutWithFrame:(CGRect)frame {
    ...
    if (self.isLayoutRequested || !self.firstLayoutRequested || changed) {
        ...
        if (!self.firstLayoutRequested) {
            [self setFirstLayoutRequested:YES];
        }
    }
    ...
    /*if (self.isLayoutRequested || !self.firstLayoutRequested || changed) {
        ...
        if (identifier != nil) {
            NSLog(@"%@ (%@) Ignored layout ", NSStringFromClass([self class]), identifier);
        } else {
            NSLog(@"%@ Ignored layout ", NSStringFromClass([self class]));
        }
    }*/
#endif
}
